...
This process happens in private: given the same inbox, starting [**state**](../../anatomy-of-an-agent/state.md) and external [**context**](../../anatomy-of-an-agent/context.md), an agent will always end up with the same outbox and finishing state \(unless there is randomness in its behaviors\).
